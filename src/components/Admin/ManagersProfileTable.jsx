...
import '../../styles/ManagersProfileTable.css'; // CSS file with the same class names

const URL_api = 'http://localhost:5164/getmanagerspofilesdata';
const DELETE_API = 'http://localhost:5164/deletemanagerprofile';

const ManagersProfileTable = () => {
  ...
  const handleDelete = async (email) => {
    try {
      ...
      if (response.data.rStatus === 1) {
        alert('Manager profile deleted successfully');
        setRes(res.filter(manager => manager[2] !== email)); // Filter out the deleted manager from the state
      } else {
        alert(response.data.rData.rMessage || 'Failed to delete manager profile');
      }
    } catch (error) {
      console.error('There was an error deleting the manager profile:', error);
      alert('There was an error deleting the manager profile');
    }
  };
    ...
    <div className="managers-profile-page">
      ...
      <main className="managers-profile-main-content">
        <div className="managers-profile-container">
          <h2 style={{color:'blue'}}>Managers Profile List</h2>
          <table className="managers-profile-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>Image</th>
                ...
                <React.Fragment key={index}>
                  {item.map((value, idx) => (
                    <tr style={{ cursor: 'pointer' }} key={idx}>
                      ...
                      <td>{value[5]}</td>
                      <td>
                        <button onClick={() => handleDelete(value[2])}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </React.Fragment>
...
export default ManagersProfileTable;
